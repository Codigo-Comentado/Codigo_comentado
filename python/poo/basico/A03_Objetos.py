"""
=============================================================
TEMA: Utilização e Manipulação de Objetos em Python
...
Pré-requisitos: Lógica de programação básica e conhecimentos básicos sobre classes e construtores
...
organizada e intuitiva. Sem objetos, precisaríamos gerenciar dezenas de 
variáveis soltas ou dicionários complexos para representar itens do sistema, 
o que torna o código confuso, difícil de escalar e propenso a erros. Os objetos 
agrupam dados e comportamentos em uma única unidade coesa e isolada.
...
ERROS MAIS COMUNS
-------------------------------------------------------------
1. Tentar chamar um método de instância diretamente pela classe: Chamar uma função 
   diretamente na classe (ex: `Computador.ligar()`) sem passar um 
   objeto real, gerando erro de tipo.
2. Criar uma instância sem armazenar sua referência: Instanciar um objeto 
   (por exemplo, `Computador("Dell", "16GB")`) sem atribuí-lo a uma variável. 
   Embora o objeto seja criado, ele não poderá ser reutilizado posteriormente.
3. Acessar um atributo que não existe: Tentar ler ou manipular um dado 
   que nunca foi inicializado na instância, gerando um `AttributeError`.
4. Confundir instâncias: Achar que alterar um atributo em um objeto vai 
   mudar automaticamente o valor em outro objeto criado pela mesma classe.
5. Esquecer o uso do operador ponto (`.`): Tentar acessar um atributo 
   digitando seu nome solto dentro dos métodos ou fora deles, sem vinculá-lo 
   à referência correta.
...
(a Classe). Essa planta define onde ficam as paredes, as portas e os quartos, 
mas você não pode morar dentro de um pedaço de papel. Quando a construtora 
ergue a casa física em um terreno específico, ela cria uma **instância** 
desse projeto (o Objeto). Você pode construir dez casas idênticas usando 
a mesma planta original, mas cada casa física estará em um endereço 
diferente, poderá ter uma cor de parede diferente e a mobília de uma 
não interfere na da outra.

-------------------------------------------------------------
RESUMO
-------------------------------------------------------------
Objetos são instâncias de uma classe. Cada objeto possui seus próprios 
atributos e pode executar métodos definidos pela classe, permitindo 
representar entidades de forma organizada, reutilizável e independente 
...
print(carro_1.apresentar())  # Saída: Carro modelo: Fusca, Cor: Azul
print(carro_2.apresentar())  # Saída: Carro modelo: Civic, Cor: Preto

# 3. Alterando o estado de apenas um objeto sem afetar o outro
print(carro_1.pintar("Vermelho"))
print(carro_1.apresentar())  # Saída: Carro modelo: Fusca, Cor: Vermelho
print(carro_2.apresentar())  # Saída: Carro modelo: Civic, Cor: Preto (intacto!)
...
classe `Carro` e retorna uma referência para esse objeto. O mesmo acontece 
para `carro_2`. O uso do operador ponto nos permite acionar os métodos 
`apresentar` e `pintar` direcionados à instância correspondente. 
Como resultado, podemos mudar a cor do Fusca para vermelho sem que o 
Civic sofra qualquer alteração, demonstrando que cada objeto mantém seu próprio estado.
"""
...
# ERRO 1: Tentar chamar um método de instância diretamente pela classe
try:
...
# ERRO 2: Criar uma instância sem armazenar sua referência
Computador("Dell", "16GB") 


# ERRO 3: Acessar um atributo que não existe
class Pessoa:
...
nenhuma variável mantém uma referência para ele. Dessa forma, o objeto 
torna-se inacessível após sua criação e poderá ser removido futuramente 
pelo coletor de lixo (Garbage Collector), impossibilitando sua reutilização.
...
os atributos `titulo` e `autor`. Em seguida, crie um método chamado 
`detalhes` que retorne uma frase apresentando o livro. 
...
textos, listas e até mesmo as funções que você cria! Quando você escreve 
o número `10` no seu código, por trás dos panos o Python está criando 
uma instância da classe `int`, o que nos dá acesso a vários métodos 
embutidos que podemos usar diretamente neles (ex: `(10).bit_length()`). 
